...
def code_key(code):
    return (code["protocol"], code["address"], code["command"], code["raw"])
...
def remove_ambiguous_codes(play_codes, jump_codes):
    ...
    for protocol, address, command, raw in sorted(ambiguous_keys):
        print(f"  protocol={protocol} address=0x{address:X} command=0x{command:X} raw=0x{raw:X}")
...
def capture_button_samples(ser, label, command, sample_count, timeout):
    ...
    for index, code in enumerate(unique, start=1):
        print(
            ...
            f"address=0x{code['address']:X} command=0x{code['command']:X} raw=0x{code['raw']:X}"
        )
...
constexpr LearnedIrCode IR_LEARNED_{name}_CODES[] = {{
{body}
}};
"""
